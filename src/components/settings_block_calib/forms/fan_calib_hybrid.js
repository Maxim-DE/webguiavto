...
function FanCalibSettings_hybrid(props) {
  ...
  const handleClick_saveModbusType = (event) => {
    const target = event.target,
          name = target.name,
          value = name.replace('fan_calib_mode_', '')

    const request_obj = {
      address: 'calib_fan.cgi',
      data: `fan_calib_mode$${value}`,
      reducer: reducers.calibration_form,
      notifications: {
        good: 'default',
        bad: 'default'
  ...
  React.useEffect(() => {
    setFanCalibState(prevState => ({
      ...prevState,
      fan_temp_threshold_max: 0,
      fan_temp_threshold_on: 0
    }))
  }, [fanCalibState.fan_calib_mode])
...
export default FanCalibSettings_hybrid