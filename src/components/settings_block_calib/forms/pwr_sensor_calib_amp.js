...
function PWRSensorCalibSettings_AMP(props) {
  ...
  const handleFormDisable = (event) => {
    ...
    setPWRSensorCalibState(prevState => ({
      ...prevState,
      pwr_sensor_availiable: value
    }))

    const req_obj = {
      address: 'calib_pwr_sensor.cgi',
      data: `pwr_sensor_availiable$${value}`,
      reducer: reducers.calibration_form,
      notifications: {
        good: 'default',
        bad: 'default'
  ...
  return (
    <Settings_block_calib 
      header={`калибровка датчика по`}
      settings_type={`pwr_sensor_calib`}
      disabled={!PWRSensorCalibState.pwr_sensor_availiable}
...
export default PWRSensorCalibSettings_AMP