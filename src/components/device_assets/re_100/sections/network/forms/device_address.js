...
export default function Device_address_settings(props) {
  const deviceAddress_store = useSelector((store) => store.globalStore.global_data.section_data.network.device_address)
  ...
  const state_handler = (state) => {
    let target_state_clone = cloneDeep(deviceAddressState)

    for (const key in state) {
      target_state_clone[key] = state[key]
    }

    setDeviceAddressState(target_state_clone)
  }
  ...
  const handleClick_save = (event) => {
    ...
    const request_obj = {
      ...
      save_data: {
        time_settings: deviceAddressState
      }
    }
  ...
  return (
    <SettingsBlockWrap header={'адрес устройства'}
      ...
      <li
        key='subnet_mask'
        id='subnet_mask_setting'
        className="settings_item">
        <div className='item_header'>
          <label
            htmlFor={`subnet_mask_input`}
            className="settings_itemLabel">
            Маска подсети
          </label>
        </div>
